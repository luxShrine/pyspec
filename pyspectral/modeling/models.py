...
from pyspectral.types import ClassModelType
...
def get_model(
    ...
    match goal.model_type:
        case ClassModelType.CONV:
            model = ConvSpectralClassifier(c_in, goal.c_out, goal.H, goal.W).to(device)
...
class TrainSetup:
    ...
    @classmethod
    def get_train_setup(
        cls, c_in: int, device: torch.device, lr: float, wd: float, goal: ClassCfg
    ) -> TrainSetup:
        model = get_model(goal=goal, c_in=c_in, device=device)
        loss_fn, optimizer = get_loss_managers(model, lr, wd)
        return cls(loss_fn, optimizer, model)
